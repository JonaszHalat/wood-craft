...
gulp.task('copy:main.css', () => {
  const banner = `/*! HTML5 Boilerplate v${pkg.version} | ${
    pkg.license
...
gulp.task('modernizr', done => {
  modernizr.build(modernizrConfig, code => {
    fs.writeFile(
      `${dirs.dist}/js/vendor/modernizr-${
        pkg.devDependencies.modernizr
      }.min.js`,
      code,
      done
    );
...
gulp.task('lint:js', () =>
  gulp
    .src(['gulpfile.js', `${dirs.src}/js/*.js`, `${dirs.test}/*.js`])
    .pipe(plugins().jscs())
    .pipe(plugins().eslint())
...
gulp.task('build', done => {
  runSequence(['clean', 'lint:js'], 'copy', 'modernizr', done);
});
